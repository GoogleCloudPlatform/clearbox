...
from clearbox.models import probabilistic
import numpy as np
...
class BayesOptLinearModelTest(parameterized.TestCase):
  ...
  def test_fit_predict_works(
    ...
    model = probabilistic.BayesOptLinearModel(
        surrogate_model=probabilistic.BayesianRidgeModel(),
        acquisition_function=probabilistic.ExpectedImprovement(xi=1e-6),
        metric=metric,
    ...
    model = probabilistic.BayesOptLinearModel(
        surrogate_model=probabilistic.GaussianProcessModel(),
        acquisition_function=probabilistic.ExpectedImprovement(xi=1e-6),
        metric=metric,
        n_opt_steps=n_opt_steps,
        batch_size=batch_size,
    ...
    model = probabilistic.BayesOptLinearModel(
        surrogate_model=probabilistic.GaussianProcessModel(),
        acquisition_function=probabilistic.MaxMean(),
        metric=metric,
        n_opt_steps=n_opt_steps,
        batch_size=batch_size,
        seed_batch_size=seed_batch_size,
